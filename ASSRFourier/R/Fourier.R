...
FouierCSVToPlot <- function(data.path=NULL, dir=NULL , freq=c(400,5000), title="", names=NULL, sqrt.trans=TRUE){
  ...
  title = tail(strsplit(dir, split="/")[[1]], 1)
    ...
    p <- FourierReadCSV(data.path,dir) %>% FourierMeans(names=names) %>% FourierDraw(freq,title, sqrt.trans=sqrt.trans)
  }
...
#' @param sqrt.trans square root axis
#'
#' @return Saved plot
#'
...
#' @param sqrt.trans square root axist transformation for plotting
#'
#' @return An object of class ggplot.
#'
#' @importFrom ggplot2 ggplot
...
FourierAggr <- function(dir, save=FALSE, name=NULL, plots=TRUE, freq=c(400,5000), sqrt.trans=TRUE){
  ...
  #plotting
    if(plots) FourierAggregatePlot(profiles, freq, sqrt.trans)
    }else{
      return(profiles)
    }
}
